    ...
    const fetchStatements = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_URL_DEV}/api/home-owner/statements/${propId}`,
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch statements: ${await response.text()}`
          );
        }
      ...
      <main className={styles.main_content}>
                  ...
                  <td>
                    <span
                      className={`${styles.status} ${
                        statement.bll_pay_stat.trim().toLowerCase() ===
                          "paid" &&
                        statement.transactions_status === "completed"
                          ? styles.paid
        ...
        <div className={styles.pagination}>
          <div className={styles.page_info}>
            Showing {statements.length} of {statements.length} results
          </div>
          ...
          <div className={styles.modal_content_three_column}>
            ...
            <div className={styles.modal_body_three_column}>
              <div className={styles.modal_column_three}>
                <p>
                  <strong>Billing Coverage Period:</strong>{" "}
                  {selectedStatement.bll_bill_cov_period}
                </p>
                <p>
                  <strong>Water Consumption:</strong>{" "}
                  {selectedStatement.bll_water_consump?.$numberDecimal ||
                    selectedStatement.bll_water_consump}{" "}
                  m<sup>3</sup>
                </p>
                <p>
                  <strong>Water Charges:</strong> PHP{" "}
                  {selectedStatement.bll_water_charges?.$numberDecimal ||
                    selectedStatement.bll_water_charges}
                </p>
                <p>
                  <strong>Water Reading:</strong>{" "}
                  {selectedStatement.bll_water_read?.$numberDecimal ||
                    selectedStatement.bll_water_read}
                </p>
              </div>
              <div className={styles.modal_column_three}>
                <p>
                  <strong>Garbage Charges:</strong> PHP{" "}
                  {selectedStatement.bll_garb_charges?.$numberDecimal ||
                    selectedStatement.bll_garb_charges}
                </p>
                <p>
                  <strong>Homeowner Fee:</strong> PHP{" "}
                  {selectedStatement.bll_hoamaint_fee?.$numberDecimal ||
                    selectedStatement.bll_hoamaint_fee}
                </p>
                <p>
                  <strong>Total Amount Due:</strong> PHP{" "}
                  {selectedStatement.bll_total_amt_due?.$numberDecimal ||
                    selectedStatement.bll_total_amt_due}
                </p>
                <p>
                  <strong>Payment Status:</strong>{" "}
                  {selectedStatement.bll_pay_stat}
                </p>
              </div>
              <div className={styles.modal_column_three}>
                <p>
                  <strong>Proof of Water Billing:</strong>
                </p>
