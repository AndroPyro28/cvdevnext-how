        ...
        const fetchPropertiesAndUserData = async () => {
            ...
            try {
                const [propertiesResponse, userResponse] = await Promise.all([
                    await axios.get(`${process.env.NEXT_PUBLIC_URL_DEV}/api/home-owner/properties/${userId}`, {}),
                    await axios.get(`${process.env.NEXT_PUBLIC_URL_DEV}/api/home-owner/header/${userId}`, {}),
                ]);
                                ...
                                <div key={property._id} className={styles.property_card}>
                                    ...
                                    <div className={styles.property_details}>
                                        ...
                                        <p className={styles.address}>
                                            <span className={styles.address_label}>Address:</span> {property.prop_street}
                                        </p>
                                        <div className={styles.billing_label}>Billing Status</div>
                                        <div
                                            className={`${styles.billing_status} ${styles[property.prop_payment_status === 'Fully Paid' ? 'billing_full' : 'billing_partial']
                                                }`}
                                        >
                                            {property.prop_payment_status}
                                        </div>
                                        <Link href={`/properties/viewproperties/${property._id}`}>
                                            <button className={styles.button_primary}>View Property</button>
                                        </Link>
                                    </div>
                                </div>
                            ))
